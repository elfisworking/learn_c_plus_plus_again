// asynchronous socker server -- accepting multiple clients concurrently
...
// Each peer is globally identified by the file descriptor it's connected on.
...
// Thses constatns make creating fd_stauts_t values les verbose
            ...
            if(events[i].data.fd == listener_sockfd) {
                ...
                if(newsockfd < 0) {
                    if(errno = EAGAIN || errno == EWOULDBLOCK) {
                        ...
                        // case don't do anyhting, but print a  notice(since these events are extremely rare and interesting to observe ...)
                ...
                if(events[i].events & EPOLLIN) {
                    ...
                    printf("%d, %d\n", status.want_read, status.want_write);
                    struct epoll_event event = {0};
                    event.data.fd = fd;
                    if(status.want_read) {
                        event.events |= EPOLLIN;
